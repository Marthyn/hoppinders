...
* Hoppinders command line responds to `-v` and `--version` options
* Clean up `bin/rake`
...
* Improves hoppinders' test suite speed
...
* Swap `id="flash"` for `class="flashes"` in `_flashes.html.haml`
...
* Remove things in Hoppinders that Rails does for us now.
...
* Allow additional
  [Heroku flags](https://github.com/thoughtbot/hoppinders#heroku)
...
* Set up Bitters during Hoppinders setup. http://bitters.bourbon.io/
* Remove SimpleCov.
* Force Hoppinders to use a particular Rails version (4.1.1).
...
* Fix `rake dev:prime` now that Hoppinders-generated apps require some `ENV`
  variables to be set.
